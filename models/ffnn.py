...
class ffnn:
    ...
    def tune_model(self, epochs: int = 200):
        '''
    ...
    def abs_error(self, true, prediction):
    ...
    def rolling_origin_forecast(self):
        ...
        for i in range(self.test_array.shape[0] // self.num_countries):
            ...
            ######################
            self.model = self.init_model(32, 16, 0.2)
            self.model.fit(curr_train_inputs, curr_train_targets, batch_size = 16, epochs = 17, verbose = 0)
            ...
            for j in range(self.num_countries):
                ...
                # obtaining the true value of the variable at this time step
                temp_original_test_arr = self.original_test_array.reshape(self.num_countries, -1, num_features)
                true_value = temp_original_test_arr[j, i, -1]
                # obtaining the forecast error
                forecast_error = self.abs_error(reconstructed_forecast_value, true_value)
                forecasts_mae.append(forecast_error)
            ...
            print(len(forecasts_mae))
        return forecasts_mae
