...
class rnn:
    ...
    def init_model(self, gru_units_1, gru_units_2, dropout_rate, num_lags,
                   optimiser: str = 'adam', loss: str = 'mse'):
        '''
        Initialises the instance of the GRU model
        
        Args:
            gru_units_1: number of neurons in the first GRU layer
            gru_units_2: optional, number of neurons in the first GRU layer, 
            which is instantiated only if the value is passed in for this argument 
        ...
        '''
        inputs = keras.layers.Input(shape = (num_lags, self.num_features))
        features = keras.layers.GRU(gru_units_1, return_sequences = False if gru_units_2 == None else True, 
                                    dropout = dropout_rate)(inputs)
        if gru_units_2 != None:
            features = keras.layers.GRU(gru_units_2, return_state = False, dropout = dropout_rate)(features)
    ...
    def tune_model(self, epochs: int = 100):
        ...
        num_lags_list = [12, 16, 18]
        gru_units_1_list = [16, 32]
        gru_units_2_list = [None, 16]
        dropout_rate_list = [0.1, 0.2, 0.3]
        batch_size_list = [16, 32, 64]    

        best_val_loss = float('inf')

        for num_lags, gru_units_1, gru_units_2, dropout_rate, batch_size \
            in itertools.product(num_lags_list, gru_units_1_list, gru_units_2_list, 
                                 dropout_rate_list, batch_size_list):

            model = self.init_model(gru_units_1, gru_units_2, dropout_rate, num_lags)
            # re-generating data at every iteration because the number of lags changes
            train_inputs, train_targets = generate_data_rnn(train_arr, num_lags, self.num_countries)
            val_inputs, val_targets = generate_data_rnn(val_arr, num_lags, self.num_countries)
            ...
            if config_best_val_loss < best_val_loss:
                ...
                best_config = (gru_units_1, gru_units_2, dropout_rate)
        ...
        metadata = {
                    'gru_units_1': int(best_config[0]),
                    'gru_units_2': int(best_config[1]) if best_config[1] is not None else None,
        ...
        with open("..\\models_saves\\rnn\\rnn_metadata.json", "w") as f:
            json.dump(metadata, f, indent=4)

        print(f'best config is:\nnumber of lags: {best_num_lags}')
        print(f'1st GRU layer neurons: {best_config[0]}')
        print(f'2nd GRU layer neurons {best_config[1]}')
    ...
    def load_metadata(self): 
        with open('..\\models_saves\\rnn\\rnn_metadata.json', 'r') as f:
            metadata = json.load(f)
        return metadata

    def construct_model_from_metadata(self, metadata):
        gru_1 = metadata['gru_units_1']
        gru_2 = metadata['gru_units_2']
        dropout_rate = metadata['dropout_rate']
        num_lags = metadata['best_num_lags']
        model = self.init_model(gru_1, gru_2, dropout_rate, num_lags)
    ...
    def rolling_origin_forecast(self):
        ...
        for i in range(self.test_array.shape[0] // self.num_countries):
            ...
            curr_train_inputs, curr_train_targets = generate_data_rnn(normalised_train_arr, num_lags = num_lags, 
            ...
            for j, country_name in enumerate(self.list_of_countries):
                ...
                forecast_input = temp_train_arr[j, -num_lags:, :].reshape(1, num_lags, self.num_features)
                # running inference
